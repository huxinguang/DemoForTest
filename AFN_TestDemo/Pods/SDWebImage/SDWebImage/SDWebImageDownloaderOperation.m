...
- (nonnull instancetype)initWithRequest:(nullable NSURLRequest *)request
    ...
    if ((self = [super init])) {
        ...
        _options = options;//options是readonly属性，但这只是不能以setter方法为其赋值，在类内部可以通过成员变量为其赋值
...
- (void)URLSession:(NSURLSession *)session
 ...
 completionHandler:(void (^)(NSCachedURLResponse *cachedResponse))completionHandler {
    ...
    //self.options属性的值是在本类对象的初始化方法里通过成员变量来赋值的，因为options是只读属性，不能通过setter方法赋值
    /*
     UIImageView的实例对象调用sd_setImageWithURL: placeholderImage: options:来设置图片，当options参数传的是
     SDWebImageRefreshCached 时，下面的self.options才为SDWebImageDownloaderUseNSURLCache
     
     所以当sd_setImageWithURL: placeholderImage: options:传的options 不为 SDWebImageRefreshCached 时，
     下面的cachedResponse = nil，即NSURLCache中不会有缓存
     */
